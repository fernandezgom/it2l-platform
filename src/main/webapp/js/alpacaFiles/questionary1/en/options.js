var options = {
    "fields": {
    	"question1": {
        	"label": "1. I often use a computer.",
        	"type": "radio",
        	"optionLabels": ["<br/><spam class='eti'>Strongly<br/> disagree</spam>&nbsp;&nbsp;&nbsp;", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/><spam class='eti'>Strongly<br/> agree</spam>"],
        	"vertical": false
        	
    	},
    	"question2": {
        	"label": "2. I often learn maths with a computer.",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["<br/><spam class='eti'>Strongly<br/> disagree</spam>&nbsp;&nbsp;&nbsp;", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/><spam class='eti'>Strongly<br/> agree</spam>"],
        	"vertical": false
    	},
    	"question3": {
        	"label": "3. Learning with a computer is fun",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["<br/><spam class='eti'>Strongly<br/> disagree</spam>&nbsp;&nbsp;&nbsp;", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/><spam class='eti'>Strongly<br/> agree</spam>"],
        	"vertical": false
    	},
    	"question4": {
        	"label": "4. I like maths.",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["<br/><spam class='eti'>Strongly<br/> disagree</spam>&nbsp;&nbsp;&nbsp;", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/><spam class='eti'>Strongly<br/> agree</spam>"],
        	"vertical": false
    	},
    	"question5": {
        	"label": "5. I like fractions.",
        	"type": "radio",
        	"helper": "----------------------------------------------------------------------------------------------------------------",
        	"optionLabels": ["<br/><spam class='eti'>Strongly<br/> disagree</spam>&nbsp;&nbsp;&nbsp;", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/><br/>", "<br/><spam class='eti'>Strongly<br/> agree</spam>"],
        	"vertical": false
    	},
    	"question6" : {
            "label": "6. Which number line correctly shows ¾?",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["../images/alpacaImages/questionary1/q1/1.png", "../images/alpacaImages/questionary1/q1/2.png", "../images/alpacaImages/questionary1/q1/3.png", "../images/alpacaImages/questionary1/q1/4.png"],
        	"vertical": false
        },
        "question7": {
        	"label": "7. Select ALL the representations that show ⅓. You can select more than one answer!",
        	"type": "checkbox",
        	"helper": "",
        	"optionLabels": ["../images/alpacaImages/questionary1/q2/1.png", "../images/alpacaImages/questionary1/q2/2.png", "../images/alpacaImages/questionary1/q2/3.png", "../images/alpacaImages/questionary1/q2/4.png"]
    	},
    	"question8": {
        	"label": "8. Which fraction is the odd one out?",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["../images/alpacaImages/questionary1/q3/1.png", "../images/alpacaImages/questionary1/q3/2.png", "../images/alpacaImages/questionary1/q3/3.png", "../images/alpacaImages/questionary1/q3/4.png"],
        	"vertical": false
    	},
    	"question9": {
        	"label": "9. Which fraction is the odd one out?",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["../images/alpacaImages/questionary1/q4/1.png", "../images/alpacaImages/questionary1/q4/2.png", "../images/alpacaImages/questionary1/q4/3.png", "../images/alpacaImages/questionary1/q4/4.png", "../images/alpacaImages/questionary1/q4/5.png"],
        	"vertical": false
    	},
    	"question10": {
        	"label": "10. What goes in the box?",
        	"type": "select",
        	"helper": "",
        	"optionLabels": ["Choose one","4","8","9","12"]
    	},
    	"question11": {
        	"label": "11. Which of these is equivalent to 5/6 and has 18 as the denominator?",
        	"type": "radio",
        	"helper": "",
        	"optionLabels": ["../images/alpacaImages/questionary1/q6/1.png", "../images/alpacaImages/questionary1/q6/3.png", "../images/alpacaImages/questionary1/q6/4.png", "../images/alpacaImages/questionary1/q6/5.png"],
        	"vertical": false
    	}
    }
};